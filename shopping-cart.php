...
if($action=='addcart' && $_SERVER['REQUEST_METHOD']=='POST') {
	...
	$stmt->execute();
	$product = $stmt->fetch();
	$currentQty = $_SESSION['products'][$_POST['ProductID']]['qty']+1; //Incrementing the product qty in cart
	$_SESSION['products'][$_POST['ProductID']] =array('qty'=>$currentQty,'name'=>$product['name'],'image'=>$product['image'],'price'=>$product['price']);
	$product='';
...
<div class="container" style="width:600px;">
  ...
  <table class="table table-striped">
    ...
    <tr>
      ...
      <td><a href="shopping-cart.php?action=empty&ProductID=<?php print $key?>" class="btn btn-info">Delete</a></td>
    </tr>
          ...
          <div class="caption">
            ...
            <form method="post" action="shopping-cart.php?action=addcart">
              <p style="text-align:center;color:#04B745;">
                ...
                <input type="hidden" name="ProductID" value="<?php print $product['ProductID']?>">
              </p>
            </form>
          </div>
        </div>
